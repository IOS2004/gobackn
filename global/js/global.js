...
function disableFormItems(disable, group) {
  var groupClass;
  if (typeof group == "undefined") groupClass = "";
  else groupClass = ".disable-group" + group;

  if (disable === true) {
    var e = $(".config .form-item.disable" + groupClass).addClass("disabled");
    e.find("input, select")
      .filter(":not([disabled])")
      .attr("disabled", "disabled")
      .addClass("temp-disabled");
    e.find("div.ui-slider").slider("option", "disabled", true);
  } else {
    var e = $(".config .form-item.disable" + groupClass).removeClass(
      "disabled",
      300,
      "linear"
    );
...
function checkFeatures(args) {
  ...
  if (args.canvas && !Modernizr.canvas) errors.push("canvas element");

  if (errors.length == 0) return;

  var msg = $(
    '<div class="browser-features-missing"><h1>Warning</h1><p>Your Browser does not support all features needed to run this animation.</p><p>Features Missing: <ul class="features-missing"></ul></p><p>Please use a modern Browser that supports these features.</p></div>'
  );
